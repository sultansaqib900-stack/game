...
```bash
node tools/headless-test.mjs     # no dependencies: logic, ads policy, balance bots
npm i && node tools/render-shot.mjs   # renders real gameplay frames to shots/*.png
npm i && npm run shots           # full DOM screenshots (needs a local browser)
```

...
pump(1200);  // ~20s of opening ramp
assert(game.world.enemies.alive + game.world.player.kills > 0,
  'enemies spawn (' + game.world.enemies.alive + ' alive, ' + game.world.player.kills + ' killed)');
...
for (let i = 0; i < 200; i++) {
  const a = (i / 200) * 6.283, d = 520 + (i % 7) * 90;
  game.world.spawnEnemy(['grunt', 'runner', 'brute', 'spitter', 'orbiter', 'phantom', 'swarm'][i % 7],
    game.world.player.x + Math.cos(a) * d, game.world.player.y + Math.sin(a) * d);
}
pump(120);
assert(game.world.enemies.alive > 100, 'handles 100+ entities (' + game.world.enemies.alive + ')');
let g3 = 0; while (game.state === 'levelup' && g3++ < 80) game.chooseOffer(game.offers[0]);
if (game.state !== 'play') game.startRun();
game.pause(); assert(game.state === 'pause', 'pause works');
game.resume(); assert(game.state === 'play', 'resume works');

/* ── mid/late-game balance: a kiting bot that auto-drafts must survive minutes ── */
{
  game.toMenu(); game.startRun();
  const p = game.world.player;
  let frames = 0; const target = 60 * 300;
  while (frames < target && game.state === 'play') {
    const q = game.world.queryEnemies(p.x, p.y, 420);
    let cx = 0, cy = 0, n = 0;
    for (const e of q) if (!e.dead) { cx += e.x; cy += e.y; n++; }
    let ax = 1, ay = 0;
    if (n) { const a = Math.atan2(p.y - cy / n, p.x - cx / n); ax = Math.cos(a); ay = Math.sin(a); }
    // steer back toward arena center when hugging a wall
    if (p.x < 300) ax += 1.2; if (p.x > 2900) ax -= 1.2;
    if (p.y < 300) ay += 1.2; if (p.y > 2100) ay -= 1.2;
    const l = Math.hypot(ax, ay) || 1;
    game.input.move.x = ax / l; game.input.move.y = ay / l;
    if (n > 5 && p.dashCd <= 0) game.input.press('dash'); else game.input.release('dash');
    pump(1); frames++;
    let g2 = 0;
    while (game.state === 'levelup' && g2++ < 80) {
      // human-ish draft: new weapons first, then damage/cooldown passives
      const score = (o) => (o.kind === 'weapon' ? (o.isNew ? 4 : 2) : 0)
        + (['amplifier', 'overclock', 'critcore', 'multishot'].includes(o.id) ? 3 : 0)
        + (o.rarity === 'epic' || o.rarity === 'legendary' ? 1 : 0);
      const best = game.offers.slice().sort((a, b) => score(b) - score(a))[0];
      game.chooseOffer(best);
    }
  }
  console.log('  bot run → t=' + Math.round(game.time) + 's level=' + p.level + ' hp=' + Math.max(0, Math.round(p.hp)) + ' kills=' + p.kills + ' weapons=' + p.weapons.length);
  assert(game.time >= 120, 'kiting bot survives past 2:00 (t=' + Math.round(game.time) + 's)');
  assert(p.level >= 6, 'bot drafts real builds (level=' + p.level + ')');
}

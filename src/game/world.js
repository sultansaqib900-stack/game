...
export class World {
  ...
  healPlayer(v) { const p = this.player; if (p.hp >= p.maxHp) return; p.hp = Math.min(p.maxHp, p.hp + v); audio.sfx('heal'); this.ring(p.x, p.y, '#ff8fb0', 10, 40, 0.4); }
  ...
  update(dt) {
    ...
    this.updateFx(dt);
    // regen
    if ((m.regenAdd || 0) > 0) { p.regenT += dt; if (p.regenT >= 1) { p.regenT -= 1; this.healPlayer(m.regenAdd); } }
    // streak decay

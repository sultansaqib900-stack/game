...
export class Game extends Bus {
  ...
  director(dt) {
    const t = this.time;
    // present from the start (0.85/s) but readable; ramps into chaos later
    const ramp = t < 8 ? 0.45 + 0.55 * (t / 8) : 1;
    const rate = (0.85 + (t / 60) * 0.9 + Math.pow(t / 60, 2) * 0.22) * ramp;
    this.spawnAcc += dt * rate;
    while (this.spawnAcc >= 1) {
      this.spawnAcc -= 1;
      if (this.world.enemies.alive < this.aliveCap) this.spawnRing(1);
    }
    // waves arrive from a shifting direction so the flow is readable, not a surround
    this.spawnArcT = (this.spawnArcT || 0) - dt;
    if (this.spawnArcT <= 0) { this.spawnArcT = 6; this.spawnArc = rand(0, Math.PI * 2); }
  ...
  spawnRing(n) {
    const p = this.world.player;
    const view = Math.max(this.renderer.w, this.renderer.h) / 2 / this.cam.zoom + 220;
    for (let i = 0; i < n; i++) {
      const type = this.rollEnemyType();
      const a = chance(0.72) ? (this.spawnArc || 0) + rand(-0.62, 0.62) : rand(0, Math.PI * 2);
  ...
  buildOffers(count) {
    ...
    const out = [];
    const weight = (o) => {
      let w = RARITY[o.rarity].w * (o.rarity === 'epic' || o.rarity === 'legendary' ? 1 + luck : 1);
      if (this.time < 180 && o.kind === 'weapon') w *= 1.8;   // builds come online early
      return w;
    };
    ...
    // guarantee a NEW weapon in the draft until the player has 4 (genre standard)
    if (p.weapons.length < 4 && !out.some(o => o.kind === 'weapon' && o.isNew)) {
      const cand = pool.findIndex(o => o.kind === 'weapon' && o.isNew);
      if (cand >= 0) out[out.length - 1] = pool.splice(cand, 1)[0];
    }
